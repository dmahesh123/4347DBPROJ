...
def load_craft_data():
    ...
    cursor.execute("SELECT UserID, CraftName, DifficultyLevel, EstimatedTime, AgeRange FROM Craft WHERE CraftID = %s", craft_id)
...
def update_craft():
    try:
        ...
        cursor.execute("""
            UPDATE Craft
            SET UserID = %s, CraftName = %s, DifficultyLevel = %s, EstimatedTime = %s, AgeRange = %s
            WHERE CraftID = %s
